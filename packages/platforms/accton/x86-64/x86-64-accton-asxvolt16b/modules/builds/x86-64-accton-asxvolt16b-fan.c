...
static int asxvolt16b_fan_probe(struct platform_device *pdev);
static int asxvolt16b_fan_remove(struct platform_device *pdev);
...
struct asxvolt16b_fan_data *data = NULL;

static struct platform_driver asxvolt16b_fan_driver = {
    .probe      = asxvolt16b_fan_probe,
    .remove     = asxvolt16b_fan_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
enum asxvolt16b_fan_sysfs_attrs {
	FAN_ATTR(1),
    FAN_ATTR(2),
    FAN_ATTR(3),
    FAN_ATTR(4),
...
static struct attribute *asxvolt16b_fan_attributes[] = {
    /* fan attributes */
    DECLARE_FAN_ATTR(1),
    DECLARE_FAN_ATTR(2),
    DECLARE_FAN_ATTR(3),
...
static const struct attribute_group asxvolt16b_fan_group = {
    .attrs = asxvolt16b_fan_attributes,
};
...
static struct asxvolt16b_fan_data *asxvolt16b_fan_update_device(void)
{
    int status = 0;
...
static ssize_t show_fan(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = asxvolt16b_fan_update_device();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
...
static int asxvolt16b_fan_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &asxvolt16b_fan_group);
	if (status) {
		goto exit;
	}
...
static int asxvolt16b_fan_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &asxvolt16b_fan_group);

    return 0;
}

static int __init asxvolt16b_fan_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct asxvolt16b_fan_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&asxvolt16b_fan_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&asxvolt16b_fan_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit asxvolt16b_fan_exit(void)
{
...
MODULE_DESCRIPTION("ASXVOLT16B fan driver");
MODULE_LICENSE("GPL");

module_init(asxvolt16b_fan_init);
module_exit(asxvolt16b_fan_exit);
