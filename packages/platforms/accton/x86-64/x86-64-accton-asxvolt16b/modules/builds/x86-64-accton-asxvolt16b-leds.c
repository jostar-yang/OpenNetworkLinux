...
static int asxvolt16b_led_probe(struct platform_device *pdev);
static int asxvolt16b_led_remove(struct platform_device *pdev);
...
struct asxvolt16b_led_data *data = NULL;

static struct platform_driver asxvolt16b_led_driver = {
    .probe      = asxvolt16b_led_probe,
    .remove     = asxvolt16b_led_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
enum asxvolt16b_led_sysfs_attrs {
	LED_LOC,
    LED_DIAG,
    LED_PSU1,
    LED_PSU2,
...
static struct attribute *asxvolt16b_led_attributes[] = {
...
static const struct attribute_group asxvolt16b_led_group = {
    .attrs = asxvolt16b_led_attributes,
};
...
static struct asxvolt16b_led_data *asxvolt16b_led_update_device(void)
{
    int status = 0;
...
static ssize_t show_led(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = asxvolt16b_led_update_device();
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }

    //printk("data->ipmi_resp[LOC_INDEX]=%d\n", data->ipmi_resp[LOC_INDEX]);
    //printk("data->ipmi_resp[DIAG_INDEX]=%d\n", data->ipmi_resp[DIAG_INDEX]);
	...
	switch (attr->index) {
		...
		case LED_DIAG:
               {            
                  ...
                  if (data->ipmi_resp[DIAG_INDEX]==2)
    ...
    switch (attr->index) {
		...
		case LED_DIAG:
        {
            if (mode == LED_MODE_ORANGE ) {
                data->ipmi_resp[DIAG_INDEX] = 2;                
            }
            else
            {
                data->ipmi_resp[DIAG_INDEX] = 1;
            }
			break;   
        }
		default:
...
static int asxvolt16b_led_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &asxvolt16b_led_group);
	if (status) {
		goto exit;
	}
...
static int asxvolt16b_led_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &asxvolt16b_led_group);	

    return 0;
}

static int __init asxvolt16b_led_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct asxvolt16b_led_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&asxvolt16b_led_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&asxvolt16b_led_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit asxvolt16b_led_exit(void)
{
...
MODULE_DESCRIPTION("ASXVOLT16B led driver");
MODULE_LICENSE("GPL");

module_init(asxvolt16b_led_init);
module_exit(asxvolt16b_led_exit);
