...
static int asxvolt16b_psu_probe(struct platform_device *pdev);
static int asxvolt16b_psu_remove(struct platform_device *pdev);
...
struct asxvolt16b_psu_data *data = NULL;

static struct platform_driver asxvolt16b_psu_driver = {
    .probe      = asxvolt16b_psu_probe,
    .remove     = asxvolt16b_psu_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
enum asxvolt16b_psu_sysfs_attrs {
	/* psu attributes */
    PSU_ATTR(1),
    PSU_ATTR(2),
    NUM_OF_PSU_ATTR,
...
static struct attribute *asxvolt16b_psu_attributes[] = {
    /* psu attributes */
    DECLARE_PSU_ATTR(1),
    DECLARE_PSU_ATTR(2),
    NULL
...
static const struct attribute_group asxvolt16b_psu_group = {
    .attrs = asxvolt16b_psu_attributes,
};
...
static struct asxvolt16b_psu_data *asxvolt16b_psu_update_device(struct device_attribute *da)
{
...
static ssize_t show_linear(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = asxvolt16b_psu_update_device(da);
    if (!data->valid[pid]) {
        error = -EIO;
        goto exit;
    }
	...
	switch (attr->index) {
		...
		case PSU2_VOUT:
            ...
            //data->ipmi_resp[pid].status[PSU_VOUT0]=0x4;
           // data->ipmi_resp[pid].status[PSU_VOUT1]=0xd3;
            //printk("(u16)data->ipmi_resp[%d].status[PSU_VOUT0] =0x%x\n", pid, (u16)data->ipmi_resp[pid].status[PSU_VOUT0]);
            //printk("(u16)data->ipmi_resp[%d].status[PSU_VOUT1]<<8 =0x%x\n", pid ,(u16)data->ipmi_resp[pid].status[PSU_VOUT1]<<8);
...
static ssize_t show_string(struct device *dev, struct device_attribute *da, char *buf)
{
	...
	switch (attr->index) {
		...
		case PSU2_MODEL:
            ...
            //printk("data->ipmi_resp[%d].model=%s\n", pid, data->ipmi_resp[pid].model);
            len=MODEL_NAME_LEN;
            //snprintf(buf, MODEL_NAME_LEN, "%s\n", str);
			break;
		case PSU1_SERIAL:
		case PSU2_SERIAL:
            VALIDATE_PRESENT_RETURN(pid);
            ...
            len=SERIAL_NUM_LEN;
            //printk("data->ipmi_resp[%d].serial=%s\n", pid, data->ipmi_resp[pid].serial);
			break;
		default:
			error = -EINVAL;
            goto exit;
...
static int asxvolt16b_psu_probe(struct platform_device *pdev)
{
	...
	status = sysfs_create_group(&pdev->dev.kobj, &asxvolt16b_psu_group);
	if (status) {
		goto exit;
	}
...
static int asxvolt16b_psu_remove(struct platform_device *pdev)
{
    sysfs_remove_group(&pdev->dev.kobj, &asxvolt16b_psu_group);
    return 0;
}

static int __init asxvolt16b_psu_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct asxvolt16b_psu_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&asxvolt16b_psu_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&asxvolt16b_psu_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit asxvolt16b_psu_exit(void)
{
...
MODULE_DESCRIPTION("ASXVOLT16B PSU driver");
MODULE_LICENSE("GPL");

module_init(asxvolt16b_psu_init);
module_exit(asxvolt16b_psu_exit);
