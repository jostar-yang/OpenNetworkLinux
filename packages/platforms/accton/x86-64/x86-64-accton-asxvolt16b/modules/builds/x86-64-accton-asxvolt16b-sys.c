...
static int asxvolt16b_sys_probe(struct platform_device *pdev);
static int asxvolt16b_sys_remove(struct platform_device *pdev);
...
struct asxvolt16b_sys_data *data = NULL;

static struct platform_driver asxvolt16b_sys_driver = {
    .probe      = asxvolt16b_sys_probe,
    .remove     = asxvolt16b_sys_remove,
    .driver     = {
        .name   = DRVNAME,
        .owner  = THIS_MODULE,
    },
...
enum asxvolt16b_sys_sysfs_attrs {
...
static struct attribute *asxvolt16b_sys_attributes[] = {
...
static const struct attribute_group asxvolt16b_sys_group = {
    .attrs = asxvolt16b_sys_attributes,
};
#endif
...
#if 0
static struct asxvolt16b_sys_data *asxvolt16b_sys_update_cpld_ver(unsigned char cpld_addr)
{
    int status = 0;
...
static ssize_t show_cpld_version(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    data = asxvolt16b_sys_update_cpld_ver(addr);
    if (!data->valid) {
        error = -EIO;
        goto exit;
    }
...
static int asxvolt16b_sys_probe(struct platform_device *pdev)
{
	...
	/* Register sysfs hooks */
#if 0
	status = sysfs_create_group(&pdev->dev.kobj, &asxvolt16b_sys_group);
	if (status) {
		goto exit;
	}
#endif
...
static int asxvolt16b_sys_remove(struct platform_device *pdev)
{
    ...
    //sysfs_remove_group(&pdev->dev.kobj, &asxvolt16b_sys_group);

    return 0;
}

static int __init asxvolt16b_sys_init(void)
{
    int ret;

    data = kzalloc(sizeof(struct asxvolt16b_sys_data), GFP_KERNEL);
    if (!data) {
        ret = -ENOMEM;
        goto alloc_err;
    }
    ...
    ret = platform_driver_register(&asxvolt16b_sys_driver);
    if (ret < 0) {
        goto dri_reg_err;
    }
...
dev_reg_err:
    platform_driver_unregister(&asxvolt16b_sys_driver);
dri_reg_err:
    kfree(data);
alloc_err:
    return ret;
...
static void __exit asxvolt16b_sys_exit(void)
{
...
MODULE_DESCRIPTION("ASXVOLT16B System driver");
MODULE_LICENSE("GPL");

module_init(asxvolt16b_sys_init);
module_exit(asxvolt16b_sys_exit);
